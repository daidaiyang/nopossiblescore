...
public class TwoLineChartView extends View {
    ...
    private int lineColor = Color.parseColor("#42E2CC");//曲线颜色
    private float lineWidthDP = 8f;//曲线宽度dp

    private int pointColor = Color.parseColor("#00ffffff");//锚点颜色
    ...
    private int pointTextColor = Color.parseColor("#000000");//锚点文本颜色
    ...
    public TwoLineChartView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TwoLineChartView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        int width = tablePadding + getTableEnd() + getPaddingLeft() + getPaddingRight()+50;//计算自己的宽度
    ...
    private void drawLine(Canvas canvas) {
        ...
        linePaint.setColor(Color.parseColor("#42E2CC"));
        if (isPlayAnim) {
            Path dst = new Path();
    ...
    private void drawLine2(Canvas canvas) {
        linePaint.setStrokeJoin(Paint.Join.ROUND);
        linePaint.setStrokeCap(Paint.Cap.ROUND);
        linePaint.setColor(Color.parseColor("#F86B2F"));
        if (isPlayAnim) {
            Path dst = new Path();
    ...
    private void drawLinePoints(Canvas canvas) {
        ...
        for (int i = 0; i < pointCount; i++) {
            Point point = linePoints[i];
            if (point == null) break;
            pointPaint.setColor(Color.parseColor("#DDDDDD"));
            canvas.drawRect(point.x-1.5f,point.y,point.x+1.5f,0,pointPaint);
            canvas.drawCircle(point.x,0,pointWidth,pointPaint);
            drawLinePointText(canvas, String.valueOf(data.get(i)), point.x, dip2px(20));
        }
    }
    ...
    private void drawLineText(Canvas canvas) {
        if (isTwoLine) {
            ...
            for (int i = 0; i < pointCount; i++) {
                ...
                if (point == null) break;
                //绘制点的文本
                drawLinePointText(canvas, String.valueOf(dataList2.get(i).getValue()), point.x, point.y);
            }
        }
    ...
    private void setupLine() {
        if (isTwoLine){
            ...
            Point pre = new Point();
            pre.set(stepTemp+20, -getValueHeight(dataList2.get(0).getValue()));//坐标系从0,0默认在第四象限绘制
            linePoints2[0] = pre;
        ...
        int stepTemp = getTableStart();
        Point pre = new Point();
        pre.set(stepTemp+20, -getValueHeight(dataList.get(0).getValue()));//坐标系从0,0默认在第四象限绘制
        linePoints[0] = pre;
     ...
     * @param dataList
     */
    public void setData(List<Data> dataList, List<Data> datalist2, List<String> data) {
        int maxValue2 = 0;
        int minValue2 = 0;
        if (dataList == null||data == null) {
            throw new RuntimeException("dataList cannot is null!");
        }
        if (dataList.isEmpty()||data == null) return;
        this.dataList.clear();
        ...
        if (datalist2 ==null||datalist2.isEmpty()){
            ...
            maxValue2 = Collections.max(this.dataList2, new Comparator<Data>() {
                @Override
            ...
            minValue2 = Collections.min(this.dataList2, new Comparator<Data>() {
                @Override
                public int compare(Data o1, Data o2) {
                    return o1.getValue() - o2.getValue();
                }
        ...
        minValue = minValue<minValue2?minValue:minValue2;
